...
parameter int guard_bits= 10; // number of guard bits (additional bitwidth of the internal arithmetic
input clock, reset;
...
S12: begin
  sin_out <= sin[W1-1 : guard_bits] + 0.5*(2**W1); // + sin[guard_bits-1];
  cos_out <= cos[W1-1 : guard_bits] + 0.5*(2**W1); // + cos[guard_bits-1];
  
//  sin_out <= sin[W1-1 : guard_bits] + 0.5*(2**W1); // rounding
//  cos_out <= cos[W1-1 : guard_bits] + 0.5*(2**W1); // rounding
